...
def runLunarLander(number_of_episodes):
	...
	ROWS = 64
	COLS = 64
	CHANNELS = 1;#model = keras.models.load_model("player.mod")
	...
	for i in range(0, number_of_episodes):
		...
		#model = pickle.load(open('finalized_model.mod', 'rb'))
		...
		while True:
			...
			a=model.predict_classes(s.reshape(1,-1))
			a = a[0]
			# Step on the game
			...
			if done: 
				total_rewards.append(total_reward); break;
